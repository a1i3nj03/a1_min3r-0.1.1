...
extern "C" int scanhash_x16r(int thr_id, struct work* work, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	if (!init[thr_id])
	{
		...
		//if throughput >= (-i 22) error in this function vvv
//		if (throughput2intensity(throughput) > 21) Error, though TBD how broken it is after
...
//		((uint32_t*)pdata)[1] = 0xa8194c3b;//0xa8194c3b;
//		((uint32_t*)pdata)[2] = 0x7f5d6e82;//0x7f5d6e|a|2;
//		((uint32_t*)pdata)[1] = 0xaaaaaaaa;//0xa8194c3b;
//		((uint32_t*)pdata)[2] = 0xaaaaaaaa;//0x7f5d6ea2;
	...
	do {
		...
		for (int i = 1; i < 16; i++)
		{
			...
			case BMW:
				...
				break;
			case GROESTL:
				//! optimizing, so -1 flag used to run special code (if GPU is new enough)
				quark_groestl512_cpu_hash_64(thr_id, throughput, pdata[19], NULL, d_hash[thr_id], -1); order++;
				TRACE("groestl:");
				break;
			case JH:
			...
			case ECHO:
				...
				break;
			case HAMSI:
				//! Using alexis kernel instead to check for bugs
